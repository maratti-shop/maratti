...
def kb_main():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("👥 Пригласить друга", callback_data="cb_ref"),
         InlineKeyboardButton("📋 Команды", callback_data="cb_help")],
        [InlineKeyboardButton("📞 Поддержка", url="https://t.me/+79894762089")],
    ])
...
async def order_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    """
    # Подтягиваем свежие заказы с GitHub
...
async def handle_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if text.startswith('NEW_ORDER:') or text.startswith('SAVE_ORDER:'):
        try:
            ...
            if str(order.get('id','')) not in ids:
                orders.append(order)
                save_orders(orders)
                await sync_to_github()
                logger.info(f'Order saved: {order.get("num","")}')
        except Exception as e:
            logger.error(f'NEW_ORDER: {e}')
        return

    # Обработка заказа — старый формат ORDER_DATA
    if text.startswith('📦ORDER_DATA:'):
        try:
            import json as _json
            order_data = _json.loads(text[len('📦ORDER_DATA:'):])
            # Загружаем текущие заказы с GitHub
            await sync_orders_from_github()
            orders = load_orders()
            # Проверяем нет ли дублей
            existing_ids = [str(o.get('id','')) for o in orders]
            if str(order_data.get('id','')) not in existing_ids:
                orders.append(order_data)
                save_orders(orders)
                # Сразу заливаем на GitHub
                await sync_to_github()
                logger.info(f'Order saved: #{order_data.get("num","")}')
            return
        except Exception as e:
            logger.error(f'handle ORDER_DATA: {e}')
        return
...
async def stats_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    active_30 = sum(1 for u in users.values()
                    ...
                    u.get('joined_iso','') > (datetime.now()-timedelta(days=30)).isoformat())
...
async def segment_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if seg == 'bought':
        ...
        result = [u for u in users.values()
                  if u.get('joined_iso','2020-01-01')[:10] < cutoff and not u.get('blocked')]
...
async def sync_orders_from_github():
    """Загружает orders.json с GitHub на Pella (чтобы бот видел заказы из мини-приложения)"""
    if not GH_TOKEN:
        return
    try:
        data, _ = await gh_get_file(ORDERS_FILE)
        if data is not None and isinstance(data, list):
            save_orders(data)
            logger.info(f'Orders synced from GitHub: {len(data)} orders')
    except Exception as e:
        logger.warning(f'sync_orders_from_github: {e}')

async def sync_to_github():
    """Синхронизирует users.json, orders.json, bot_stats.json на GitHub"""
    if not GH_TOKEN:
        return
    files_to_sync = [
        (USERS_FILE, 'sync users'),
        (ORDERS_FILE, 'sync orders'),
        (STATS_FILE, 'sync stats'),
        (BROADCASTS_FILE, 'sync broadcasts'),
    ]
    for filename, msg in files_to_sync:
        data = load_json(filename)
        if data is None:
            continue
        try:
            _, sha = await gh_get_file(filename)
            await gh_put_file(filename, data, sha, msg)
            logger.info(f'Synced {filename} to GitHub')
        except Exception as e:
            logger.warning(f'sync_to_github {filename}: {e}')
...
async def sync_orders_job(ctx: ContextTypes.DEFAULT_TYPE):
    """Загружает свежие заказы с GitHub каждые 5 минут"""
    await sync_orders_from_github()
...
async def post_init(app: Application):
    ...
    await app.bot.set_my_commands([
        ...
        BotCommand('promo',  '🎁 Промокод на скидку'),
...
def main():
    ...
    # Синхронизация данных на GitHub каждые 30 минут
    jq.run_repeating(sync_job, interval=1800, first=120)
    # Синхронизация заказов с GitHub каждые 5 минут
    jq.run_repeating(sync_orders_job, interval=300, first=30)
